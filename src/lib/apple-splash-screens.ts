// iOS does not read the web app manifest for launch screens, so an installed
// PWA opens to a blank `background_color` field unless we hand Safari a
// pre-rendered image per device via `<link rel="apple-touch-startup-image">`.
// Next emits those links from `metadata.appleWebApp.startupImage`.
//
// Each entry pairs a PNG in `public/splash/` (the Woodshed icon centered on
// #09090b, matching the manifest background) with the media query iOS Safari
// uses to pick it: device-width/device-height are the CSS-point size of the
// device in portrait and stay fixed across orientations, so the portrait and
// landscape assets differ only by the `orientation` keyword and by having their
// pixel dimensions swapped.
//
// Regenerate with scripts/gen-splash.mjs whenever the icon artwork changes or a
// new device resolution needs covering.
